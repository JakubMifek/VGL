...
open class Block(val block: Blocks, val props: HashMap<String, Any> = hashMapOf()) {
    fun serialize(): String {
        return "${block.id}${if (props.size > 0) VALUE_SEPARATOR else ""}${
            props.map { "${it.key}$PROPS_SEPARATOR${it.value}" }.joinToString("$VALUE_SEPARATOR")
        }"
    }
